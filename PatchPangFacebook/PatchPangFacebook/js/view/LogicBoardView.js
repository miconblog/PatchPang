(function() {
	...
	LogicBoardView.prototype.moveBlocks = function(callback) {
		...
		_(this.model.block).each( function(v, i) {
			...
			var y = Math.floor(i / 7), cy = v.get("y");
			...
			if (y * SIZE !== cy) {
				...
				collie.Timer.transition(v, 200, {
					...
					onComplete : (function(obj, fCallBack){
						
						return function(){
							obj.set("isMoving", false);
	...
	LogicBoardView.prototype.switchBlocks = function(p1, p2, rollback) {
		...
		if (Math.abs(p1 - p2) === 1) {// 좌우 이동
			...
			collie.Timer.transition(function(oEvent) {
				view.model.block[p1].set({
					"x" : oEvent.value[0],
					"isMoving" : true
				});
				view.model.block[p2].set({
					"x" : oEvent.value[1],
					"isMoving" : true
				});
				...
				from : [x1, x2],
				to : [x2, x1],
				onComplete : (function(po1, po2, isRollBack) {
					
					return function(){
						view.model.block[po1].set("x", x2);
						view.model.block[po1].set("isMoving", false);
						view.model.block[po2].set("x", x1);
						view.model.block[po2].set("isMoving", false);
						view.model.switchedPos(po1, po2, isRollBack);
						view.isMovingBlocks = false;
						view.validBlocks();
					}
			...
			collie.Timer.transition(function(oEvent) {
				...
				view.model.block[p1].set("isMoving", true);
				view.model.block[p2].set("isMoving", true);
				...
				onComplete : (function(po1, po2, isRollBack) {
					...
					return function(){
						view.model.block[po1].set("y", y2);
						view.model.block[po2].set("y", y1);
						view.model.block[po1].set("isMoving", false);
						view.model.block[po2].set("isMoving", false);
						view.model.switchedPos(po1, po2, isRollBack);
						view.isMovingBlocks = false;
		...
		if (!this.userEvent) {
			this.userEvent = {
				"mousedown" : function(e) {
					...
					// 움직이고 있는 녀석은 선택 안됨!!
					console.log("움직이고 있는 녀석이니? ", e.displayObject.get("isMoving"))
					if( e.displayObject.get("isMoving") ) {
